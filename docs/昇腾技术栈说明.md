...
在赛事昇腾算力环境（Linux + CANN + NPU）执行：

```bash
bash scripts/deploy_ascend.sh
```

脚本自动完成 7 步：检测 Python → 安装 MindSpore(昇腾) → 验证 NPU → 装依赖 → 配置 ASCEND_API_KEY（读环境变量或手动编辑 .env）→ 验证昇腾加速生效（`scripts/verify_ascend.py`）→ **跑通一次真实金融分析（留存证据）** → 提示启动网站。

手动部署（等价于脚本，供理解）：

```bash
# 1. 安装 MindSpore (昇腾 NPU 版本)
...
# 4. 跑通真实代码 + 一次真实分析 (留存日志证据)
python3 scripts/verify_ascend.py
python3 -c "from src.pipeline import run_analysis; r = run_analysis('低空经济', use_llm=True, enrich_market=True); print('股票数:', len(r.get('stock_results', [])), '报告:', len(r.get('report','')), '字')"

# 5. 启动网站 (昇腾后端自动启用)
python3 -m streamlit run ui/app.py --server.port 8532
