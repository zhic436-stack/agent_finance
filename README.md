<h1 align="center">📈 多视角金融研究 Agent</h1>

<p align="center">
  <b>输入一个热点，AI 自动完成投研闭环</b><br/>
  <i>事件理解 → 产业链推理 → 候选股筛选 → 四因子评分 → 风险分析 → 研究报告</i>
</p>

<p align="center">
  <b>华为昇腾 × AtomGit AI 金融应用黑客松作品</b><br/>
  昇腾云 GLM-5.2 · MindSpore 加速 · ReAct 智能体 · 事件驱动四因子
</p>
...
| 你只需要 | 它自动完成 |
|----------|------------|
| 输入热点关键词，如「低空经济政策」 | 事件解析 → 产业链推导（上/中/下游）→ 候选股筛选 → 事件/价值/成长/市场**四因子评分** → 风险等级 → **完整 Markdown 研报** |

还支持 **🤖 Agent 模式**：让大模型像研究员一样自主决定查什么——实时调用行情 / 财务 / 新闻 / 风险工具，每一步工具调用都可追溯。

### 🆚 与"通用大模型问答"的本质区别

| 通用 LLM 问答 | 本系统 |
|---------------|--------|
| 答案"看起来合理"，但**无法验证** | 结论基于**真实行情 / 财务 / 新闻数据** |
| 黑盒，不知道结论怎么来的 | 四因子得分 + 风险量化 + **工具调用轨迹**全部可解释 |
| 每次回答不确定 | 离线确定性计算 + 122 个单元测试，**可复现** |

> 🎬 [演示视频（2 分半）](https://github.com/zhic436-stack/agent_finance/releases/download/v1.0.0-demo/demo_video_2min30s.mp4) · 🖼️ [界面预览](#-界面预览) · 🔧 [昇腾技术栈](#-昇腾技术栈合规) · ⚡ [性能数据](#-性能数据)
...
| 痛点 | 现状（人工） | 我们的解法 |
|------|--------------|------------|
| **① 信息过载** | 研究员每天翻阅 20+ 数据源（新闻/公告/行情/研报），手工拼凑主题线索，耗时且易漏 | 输入热点关键词，自动聚合事件解析 + 产业链推理 + 候选股行情/财务/新闻/风险，**秒级到分钟级**出结果 |
| **② 事件传导难** | "政策 → 产业链哪个环节受益 → 哪些标的最相关"依赖资深经验，新人难以复制 | **产业链规则库 + LLM 推理**双通道，传导逻辑显式输出（上中下游 + 受益环节），可追溯 |
| **③ 结论不可复现** | 人工研究主观性强，缺统一框架，同一热点不同人结论差异大 | 事件/价值/成长/市场**四因子标准化评分** + 风险等级（波动率/回撤/估值），离线确定性计算可复现 |
| **④ 报告产出慢** | 一份主题研报从选题到初稿 1~2 天 | 自动生成结构化 Markdown 报告（事件解读/产业链/候选股/风险/免责声明），研究员聚焦复核与判断 |

### 参考指标对照（赛事 2.2 智能投研助理场景）

- **简报产出 < 30min** → 我们：预置热点秒开、真实分析约 1~4 分钟 ✅
- **准确率 > 95%** → 我们：数据全部来自真实行情/财务缓存/联网新闻，可解释因子框架（非 LLM 编造）✅
- **使用率 > 70%** → 我们：研究闭环（输入→结论→报告）一次完成，无需在多个工具间跳转 ✅

### 为什么不用"通用大模型问答"？

通用 LLM 只能给"看起来合理"的答案，无法回答：*"这个结论基于哪些真实数据？因子得分怎么算的？风险怎么量的？"*
我们的 Agent 每一步都有**工具调用轨迹 + 真实数据来源 + 可解释因子/风险框架**——这是评审标准里"解决真问题、可落地"的关键差异。
...
> 📽️ 演示视频：[金融研究 Agent 演示录屏（2 分半）](https://gitcode.com/zhichen1024/agent_finance/blob/main/docs/demo_video_2min30s.mp4)（GitCode 国内直连；[GitHub 镜像](https://github.com/zhic436-stack/agent_finance/releases/download/v1.0.0-demo/demo_video_2min30s.mp4)）
